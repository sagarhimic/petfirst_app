...
def send_reschedule_booking_notification(
    db: Session,
    booking_id: int,
    ...
    if user_type == "trainer":
        ...
        template_id = 3
        purpose = "PET Trainer Confirmed Reschedule Booking"
        link = f"trainer/get-booking-details/{booking_id}"
        title = (
            f"Dear {name}, customer was Booking was rescheduled.Please check with customer bookingID:- {booking_id} "
        ...
        title = (
            f"Dear {name}, your booking has been successfully rescheduled. "
            f"Booking ID - {booking_id}"
        )
