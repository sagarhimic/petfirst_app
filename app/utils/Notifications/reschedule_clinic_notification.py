...
def send_reschedule_clinic_booking_notification(
    db: Session,
    booking_id: int,
    ...
    if user_type == "doctor":
        ...
        template_id = 12
        purpose = "PET Doctor Confirmation for Reschedule Booking"
        link = f"franchises/get-booking-details/{booking_id}"

        title = (
            f"Dear {name}, Customer Rescheduled booked {type_name} appointment "
            f"- {booking_id}"
        )
    ...
    # ===================================================
        ...
        name = customer.name
        template_id = 11
        purpose = "PET Customer Doctor Booking Reschedule Confirmed"
        link = f"customer/get-booking-details/{booking_id}"

        title = (
            f"Dear {name}, you confirmed Reschedule Clinic Appointment"
            f"- {booking_id}"
        )
